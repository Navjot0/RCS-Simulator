...
# No -Xms/-Xmx previously meant HotSpot ergonomics: Xms defaults to ~1/64 of
...
# accept() on the server socket. That pause is exactly what was producing
# the tight burst of java.net.ConnectException seen only in the first couple
# of seconds of every load test (never later, no matter how high sustained
# TPS climbed): connections arriving while the JVM is paused for a resize
# sit in the OS backlog until it's full, then get refused outright, with
# this application never getting a chance to log or handle them. Raising
# accept-count and ramping the load-test client's own connection burst both
# shrank that window but didn't remove its cause - this does.
#
